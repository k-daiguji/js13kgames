let can: HTMLCanvasElement | undefined;
let con: CanvasRenderingContext2D | undefined;
...
function Main() {
    can = <HTMLCanvasElement>document.getElementById('can') ?? undefined;
...
function DrawAisle(x1: number, y1: number, x2: number, y2: number) {
    if (con) {
...
function DrawWall(x1: number, y1: number, x2: number, y2: number) {
    if(con){
